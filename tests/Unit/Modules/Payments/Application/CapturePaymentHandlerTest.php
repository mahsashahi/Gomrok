...
final class CapturePaymentHandlerTest extends TestCase
{
    ...
    #[Test]
    public function capturesARenewalOriginatedPaymentByPaymentIdViaTheSubscriptionPaymentLink(): void
    {
        $paymentId = $this->seedRenewalPayment(PaymentStatus::Authorized);
        $this->adapter->captureResult(new ProviderPaymentResult('pi_renewal_1', '', 'succeeded'));

        $result = $this->handler->handle(new CapturePaymentCommand(clientId: self::CLIENT, paymentId: $paymentId));

        self::assertTrue($result->isOk());
        $value = $result->value();
        \assert($value instanceof CapturePaymentResult);
        self::assertSame('paid', $value->status);
        self::assertSame('pi_renewal_1', $value->providerReference);
        self::assertSame('pi_pending_renewal_1', $this->adapter->lastCaptureReference);
    ...
    #[Test]
    public function rejectsCaptureOnARenewalPaymentWhenTheProviderDoesNotSupportIt(): void
    {
        $this->handler = $this->buildHandler('mollie');
        $paymentId = $this->seedRenewalPayment(PaymentStatus::Authorized);
    ...
    private function seedRenewalPayment(PaymentStatus $status): int
    {
        ...
        $this->gatewayReferences->save(GatewayReference::forPayment(self::CLIENT, self::PROVIDER_ACCOUNT, GatewayReferenceType::PaymentIntent, 'pi_pending_renewal_1', $paymentId, $this->now));
