...
/**
 * Create a real Mollie payment for a provider account and print the checkout
 * URL (Phase 22 — standalone adapter demonstration, mirrors
 * bin/StripeCreateCheckoutSession.php; the payment creation flow that wires
 * this into the Payments module is Phase 24). Requires the account to already
 * hold a real Mollie test/live API key
 * (`composer provider-account:create --provider-type=mollie ...`).
 *
 *   php bin/MollieCreateCheckoutSession.php --client=televika --account=mollie-live \
 ...
 *       [--customer-email=user@example.com] [--payment-method=card|paypal|ideal|bancontact|sepa_direct_debit]
 */
...
$opts = getopt('', [
    ...
    'success-url:', 'cancel-url:', 'customer-email::', 'payment-method::',
]);
if (
    $opts === false
    ...
    fwrite(STDERR, "usage: php bin/MollieCreateCheckoutSession.php --client=<slug|id> --account=<account-slug> --attempt=<ref> --amount-minor=<n> --currency=<ISO> --description=<text> --success-url=<url> --cancel-url=<url> [--customer-email=] [--payment-method=]\n");
    exit(2);
}
...
$paymentMethod = null;
$paymentMethodOpt = $strOrNull('payment-method');
if ($paymentMethodOpt !== null) {
    $paymentMethod = PaymentMethod::tryFrom($paymentMethodOpt);
    if ($paymentMethod === null) {
        fwrite(STDERR, "error: unknown payment method '{$paymentMethodOpt}'\n");
        exit(2);
    }
}
...
fwrite(STDOUT, "payment {$result->providerReference} [{$result->rawStatus}]\n{$result->redirectUrl}\n");
exit(0);
