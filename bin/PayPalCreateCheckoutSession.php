...
/**
 * Create a real PayPal order for a provider account and print the approve
 * URL (Phase 22 — standalone adapter demonstration, mirrors
 * bin/StripeCreateCheckoutSession.php / bin/MollieCreateCheckoutSession.php;
 * the payment creation flow that wires this into the Payments module is
 * Phase 24). Requires the account to already hold real PayPal REST API
 * credentials, stored as a JSON secret:
 * `composer provider-account:create --provider-type=paypal --secret-key='{"client_id":"...","client_secret":"..."}' ...`
 *
 *   php bin/PayPalCreateCheckoutSession.php --client=televika --account=paypal-live \
...
if (
    ...
    fwrite(STDERR, "usage: php bin/PayPalCreateCheckoutSession.php --client=<slug|id> --account=<account-slug> --attempt=<ref> --amount-minor=<n> --currency=<ISO> --description=<text> --success-url=<url> --cancel-url=<url> [--customer-email=]\n");
    exit(2);
}
...
fwrite(STDOUT, "order {$result->providerReference} [{$result->rawStatus}]\n{$result->redirectUrl}\n");
exit(0);
