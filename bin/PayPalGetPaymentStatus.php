...
/**
 * Look up a PayPal order's status.
 *
 *   php bin/PayPalGetPaymentStatus.php --client=televika --account=paypal-live --reference=<order-id>
 */
...
if ($opts === false || !isset($opts['client'], $opts['account'], $opts['reference'])) {
    fwrite(STDERR, "usage: php bin/PayPalGetPaymentStatus.php --client=<slug|id> --account=<account-slug> --reference=<order-id>\n");
    exit(2);
}
