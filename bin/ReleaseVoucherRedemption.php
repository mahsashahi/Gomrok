...
/**
 * Free a reservation after a failed / canceled / expired payment attempt.
 *
 *   php bin/ReleaseVoucherRedemption.php --client=televika --voucher=1 --attempt=order-42
 */
...
if ($opts === false || !isset($opts['client'], $opts['voucher'], $opts['attempt']) || !is_string($opts['voucher']) || !ctype_digit($opts['voucher'])) {
    fwrite(STDERR, "usage: php bin/ReleaseVoucherRedemption.php --client=<slug|id> --voucher=<id> --attempt=<ref>\n");
    exit(2);
}
...
$handler = $container->get(ReleaseVoucherRedemptionHandler::class);
assert($handler instanceof ReleaseVoucherRedemptionHandler);
...
fwrite(STDOUT, "released redemption for attempt '{$asString($opts['attempt'])}'\n");
exit(0);
