<?php

declare(strict_types=1);

/**
 * Define a country → provider routing group for a client.
 *
 *   php bin/CreateProviderGroup.php --client=televika --name="Turkey" [--slug=turkey]
 *       [--default] [--device=web|ios|android] [--currency=TRY]
 */

use Gomrok\Bootstrap\ContainerFactory;
use Gomrok\Modules\Clients\Application\ClientDirectory;
use Gomrok\Modules\Providers\Application\CreateProviderGroup\CreateProviderGroupCommand;
use Gomrok\Modules\Providers\Application\CreateProviderGroup\CreateProviderGroupHandler;
use Gomrok\Modules\Providers\Application\CreateProviderGroup\CreateProviderGroupResult;

require dirname(__DIR__) . '/vendor/autoload.php';

$opts = getopt('', ['client:', 'name:', 'slug::', 'default', 'device::', 'currency::']);
if ($opts === false || !isset($opts['client'], $opts['name'])) {
    fwrite(STDERR, "usage: php bin/CreateProviderGroup.php --client=<slug|id> --name=<name> [--slug=<slug>] [--default] [--device=web|ios|android] [--currency=TRY]\n");
    exit(2);
}

$asString = static fn (mixed $v, string $default = ''): string => is_string($v) ? $v : $default;

$container = ContainerFactory::create();

$directory = $container->get(ClientDirectory::class);
assert($directory instanceof ClientDirectory);

$clientRef = $asString($opts['client']);
$client = ctype_digit($clientRef) ? $directory->findById((int) $clientRef) : $directory->findBySlug($clientRef);
if ($client === null) {
    fwrite(STDERR, "error: client '{$clientRef}' not found\n");
    exit(1);
}

$handler = $container->get(CreateProviderGroupHandler::class);
assert($handler instanceof CreateProviderGroupHandler);

$result = $handler->handle(new CreateProviderGroupCommand(
    clientId: $client->id,
    name: $asString($opts['name']),
    isDefault: array_key_exists('default', $opts),
    slug: ($slug = $asString($opts['slug'] ?? null)) !== '' ? $slug : null,
    deviceType: ($device = $asString($opts['device'] ?? null)) !== '' ? $device : null,
    currencyCode: ($currency = $asString($opts['currency'] ?? null)) !== '' ? $currency : null,
));

if ($result->isErr()) {
    $error = $result->error();
    fwrite(STDERR, "error [{$error->code}]: {$error->message}\n");
    exit(1);
}

$payload = $result->value();
assert($payload instanceof CreateProviderGroupResult);

fwrite(STDOUT, "provider group created\n");
fwrite(STDOUT, "  id:      {$payload->groupId}\n");
fwrite(STDOUT, "  slug:    {$payload->slug}\n");
fwrite(STDOUT, '  default: ' . ($payload->isDefault ? 'yes' : 'no') . "\n");

exit(0);
